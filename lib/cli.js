...
app
  ...
  .option('-m, --manual', 'use manual webroot verification OR')
  .option('-n, --dns', 'use manual DNS verification')
...
if (app.rawArgs.length<=2) {
  return app.help();
} else if (!app.webroot && !app.manual && !app.dns) {
  return console.log('Error: You need to use "--manual" or "--webroot <path>" or "--dns"');
} else if (!app.domains) {
...
function manualVerification(domain, path, data, done) {
  ...
  if (app.dns) {
    ...
    console.log(' _acme-challenge.'+domain+'  TXT  '+data);
  } else {
