...
function getCert(options, cb) {
  ...
  if (!~['http-01', 'dns-01'].indexOf(state.method)) {
    return cb(new Error('No valid "method" specified! Supported methods: http-01, dns-01'));
  }
  ...
  function getReadyToValidate(err, res, body) {
    ...
    if (res && res.statusCode===403) {
      log('accountKey not registered, starting registration');
    ...
    if (options.webroot) {
      ...
      if (state.method==='http-01') {
        options.challenge(state.domain, '/'+challengePath, keyAuthorization, challengeDone);
      } else if (state.method==='dns-01') {
        options.challenge(state.domain, null, Acme.dnsKey(keyAuthorization), challengeDone);
      }
    } else {
...
exports.webrootChallengeMiddleware=helper.webrootChallengeMiddleware;
